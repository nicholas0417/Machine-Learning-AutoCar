...
class NeuralNetwork(object):
    ...
    def create(self):
        ...
        self.model.load('mlp_xml/11_21_mlp.xml')
...
class DistanceToCamera(object):
    ...
    def __init__(self):
        ...
        self.v0 = 138.94993918
        self.ay = 279.916139949
...
class LCDprintDataHandler(Object):
    
    def __init__(self):
	     self = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
         self.connect(('192.168.0.110', 31500))
    def write(self, LCD_data):
        try:
            while 1:
                self.send(LCD_data)
                print LCD_data
...
class VideoStreamHandler(SocketServer.StreamRequestHandler):
    ...
    model = NeuralNetwork()
    model.create()
    text = LCDprintDataHandler()
    obj_detection = ObjectDetection()
    rc_car = RCControl()
    LCD = LCDprintDataHandler()
    # cascade classifiers
            ...
            while True:
                ...
                if first != -1 and last != -1:
                    ...
                    if sensor_data is not None and sensor_data < 30:
                        ...
                        if prediction == 0:
                            left_frame+=1
                            filename1 = "left_half/"+str(left_frame)+ ".jpg"
                            filename2 = "left_full/"+str(left_frame)+ ".jpg"
                            cv2.imwrite(filename1, half_gray)
                            cv2.imwrite(filename2, image)
                            LCD_data = "left"
                            self.LCD.write(LCD_data)
                        elif prediction == 1:                
                            right_frame+=1
                            filename3 = "right_half/"+str(right_frame)+ ".jpg"
                            filename4 = "right_full/"+str(right_frame)+ ".jpg"
                            cv2.imwrite(filename3, half_gray)
                            cv2.imwrite(filename4, image)
                            LCD_data = "right"
                            self.LCD.write(LCD_data)
                        elif prediction == 2:
                            forward_frame+=1
                            filename5 = "forward_half/"+str(forward_frame)+ ".jpg"
                            filename6 = "forward_full/"+str(forward_frame)+ ".jpg"
                            cv2.imwrite(filename5, half_gray)
                            cv2.imwrite(filename6, image)
                            LCD_data = "forward"
                            self.LCD.write(LCD_data)
                        self.stop_start = cv2.getTickCount()
                        self.d_stop_sign = 25
...
class ThreadServer(object):
    ...
    distance_thread = threading.Thread(target=server_thread2, args=('192.168.0.16', 8002))
    distance_thread.start()
    video_thread = threading.Thread(target=server_thread('192.168.0.16', 8000))
    video_thread.start()
